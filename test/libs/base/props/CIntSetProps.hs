...
prop_eq_sets_are_eq (EqOverlapSets (s, t, equal)) = withCIntSets [s, t] $ \[cs, ct] -> do
    res <- run (intseteq cs ct)
    assert (res == equal)

prop_disjoint_sets_are_disjoint :: DisjointOverlapSets Int32 -> Property
prop_disjoint_sets_are_disjoint (DisjointOverlapSets (s, t, equal)) = withCIntSets [s, t] $ \[cs, ct] -> do
    res <- run (intseteq cs ct)
    assert (res == equal)

prop_sets_union_eq_union :: UnionOverlapSets Int32 -> Property
prop_sets_union_eq_union (UnionOverlapSets (s, t, u)) = withCIntSets [s, t, u] $ \[cs, ct, cu] -> do
    cu' <- run (sunion cs ct)
...
prop_sets_intersection_eq_intersection (IntersectionOverlapSets (s, t, u)) = withCIntSets [s, t, u] $ \[cs, ct, cu] -> do
...
prop_sets_difference_eq_difference (DifferenceOverlapSets (s, t, u)) = withCIntSets [s, t, u] $ \[cs, ct, cu] -> do
