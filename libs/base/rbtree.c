...
// static struct rb_node *balance_after_delete(struct rb_node *node) {
//     if (!node) return node;
//
//     struct rb_node *left = node->left,
//                *right = node->right;
...
// static struct rb_node *grab_min(struct rb_node **min, struct rb_node *node) {
//     if (!node) return NULL;
//
//     if (node->left) {
...
// }
//
// static struct rb_node *_rbdelete(
//     void *key,
//     int (*keycmp) (void const *a, void const *b),
//     struct rb_node *node
// ) {
...
//
//     int ord = (*keycmp)(key, node->assoc.key);
//     if      (ord < 0) node->left = _rbdelete(key, keycmp, node->left);
//     else if (ord > 0) node->right = _rbdelete(key, keycmp, node->right);
//     else {
...
//     struct rb_node *node
// ) {
//     struct rb_node *root = _rbdelete(key, keycmp, node);
//     root->red = false;
//     return root;
// }
